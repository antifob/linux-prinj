...
if 3 != len(sys.argv):
    print('inject <lib.so> <pid>', file=sys.stderr)
    exit(1)
...
dlopen_addr = libc_map['from']
dlopen_addr += libc_elf.symbols['__libc_dlopen_mode']


# we could use a code cave or write/restore an arbitrary page but meh
lib_addr = wrlib(pid, lib_path.encode() + b'\x00')
...
try:
    ...
    # call r9; int 3
    wrmem(pid, r1.rip, b'\x41\xff\xd1\xcc')
    ptrace_cont(pid)
    os.waitpid(pid, 0)
