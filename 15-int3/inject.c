...
int main(int argc, const char* const* argv)
{
	...
	/* Write shellcode. */
	if (0 != wr2mem(pid, regs2.rax, buf, len)) {
	...
	if (0 != ptrace(PTRACE_CONT, pid, 0, 0)) {
		perror("Failed to let process resume");
		return 1;
	}

	/* Wait for the payload to execute int3. */
	if (pid != waitpid(pid, 0, 0)) {
		perror("Failed waiting for payload to return");
		return 1;
	}
